  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-10">
      <div className="grid grid-cols-1 gap-6">
        {/* Statistics Hero - Adaptive Glass */}
        <div className="glass rounded-[3rem] p-10 flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl relative overflow-hidden">
          <div className="relative z-10 flex-1">
             <h2 className="text-4xl font-black mb-4">Welcome back, <span className="text-indigo-600 dark:text-indigo-400">Explorer</span>.</h2>
             <p className="text-slate-500 dark:text-slate-400 font-medium mb-8 leading-relaxed max-w-sm">
               {lang 
             ...
             <div className="flex flex-wrap gap-4">
               ...
               <div className="glass px-8 py-4 rounded-2xl border border-slate-100 dark:border-white/10 flex items-center gap-3">
                 <span className="text-xl">💎</span>
                 <span className="font-black text-indigo-600 dark:text-indigo-400">{totalXP} <span className="text-[10px] text-slate-400">XP</span></span>
               </div>
             </div>
          </div>
          
          <div className="relative z-10 flex flex-col items-center gap-2 bg-white/40 dark:bg-slate-800/20 p-8 rounded-[2.5rem] border border-white dark:border-white/5 shadow-xl">
      ...
      {lang ? (
        <div className="glass rounded-[3rem] p-10 shadow-xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Certificate', icon: '📜', status: progressPercent === 100 ? 'Unlocked' : 'Locked', color: progressPercent === 100 ? 'text-emerald-500' : 'text-slate-400', path: '#' },
              { label: 'Flashcards', icon: '🎴', status: `${progress.flashcards.length} Cards`, color: 'text-indigo-500', path: '/flashcards' },
              { label: 'Snippets', icon: '📂', status: `${progress.savedSnippets.length} Saved`, color: 'text-emerald-500', path: '/cookbook' },
              { label: 'Interview', icon: '🎙️', status: 'Available', color: 'text-rose-500', path: '/interviewer' }
            ].map(tool => (
              <div 
                key={tool.label} 
                onClick={() => tool.path !== '#' && navigate(tool.path)}
                className="group glass p-6 rounded-[2rem] hover:border-indigo-500 hover:-translate-y-1 transition-all cursor-pointer"
              >
                ...
                <p className={`text-sm font-black ${tool.color}`}>{tool.status}</p>
              </div>
            ))}
          </div>
        </div>
      ) : (
        <div className="text-center py-32 glass rounded-[4rem] border-2 border-dashed border-indigo-200 dark:border-indigo-900/30">
           ...
           <button onClick={() => navigate('/select-language')} className="bg-slate-900 dark:bg-slate-100 dark:text-slate-900 text-white px-12 py-5 rounded-2xl font-black shadow-2xl hover:bg-slate-800 dark:hover:bg-white transition-all">Begin Selection</button>
        </div>
      )}
        ...
        <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-10 gap-4">
          {ACHIEVEMENTS.map(ach => {
            ...
            return (
              ...
              >
                {ach.icon}
                <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 text-[10px] py-1 px-3 rounded-lg opacity-0 group-hover/ach:opacity-100 transition-opacity whitespace-nowrap z-50 pointer-events-none font-bold">
                  {ach.title}
                </div>
              </div>
            );
