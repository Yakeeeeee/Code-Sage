  ...
  return (
    <div className="max-w-2xl mx-auto space-y-10 py-10 px-4">
      <div className="text-center space-y-4">
        ...
        <p className="text-gray-500 font-bold">Master concepts through active recall.</p>
      </div>

      {cards.length === 0 ? (
        <div className="bg-white dark:bg-gray-900 border-2 border-dashed rounded-[3rem] p-20 text-center space-y-6">
          <div className="text-7xl">🎴</div>
          <h3 className="text-2xl font-black">Your deck is empty</h3>
          <p className="text-gray-500 max-w-xs mx-auto">Generate smart flashcards based on your {progress.selectedLanguage || 'coding'} progress.</p>
          <button 
            onClick={handleGenerate} 
            disabled={isGenerating}
            className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black shadow-xl disabled:opacity-50"
          >
        ...
        <div className="space-y-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs font-black text-gray-400 uppercase">{currentIndex + 1} / {cards.length}</span>
          </div>
          ...
          >
            <div className={`relative w-full h-full transition-all duration-500 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
              {/* Front */}
              <div className="absolute inset-0 w-full h-full backface-hidden bg-white dark:bg-gray-900 border dark:border-gray-800 rounded-[3rem] p-10 flex flex-col items-center justify-center text-center shadow-xl">
                ...
                <span className="mt-8 text-xs font-bold text-gray-300">Tap to reveal answer</span>
              </div>
              {/* Back */}
          ...
          <div className="flex justify-between gap-4">
            <button onClick={prevCard} className="flex-1 bg-white dark:bg-gray-900 border p-5 rounded-2xl font-bold shadow-sm hover:bg-gray-50 transition-colors">← Previous</button>
            <button onClick={nextCard} className="flex-1 bg-white dark:bg-gray-900 border p-5 rounded-2xl font-bold shadow-sm hover:bg-gray-50 transition-colors">Next Card →</button>
          </div>

          <div className="text-center">
            <button onClick={handleGenerate} disabled={isGenerating} className="text-indigo-600 font-bold hover:underline">
